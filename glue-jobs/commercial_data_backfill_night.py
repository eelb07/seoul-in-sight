...
try:
    ...
    for area_id_str, values in processed_observed_at_dict.items():
        for value in values:
            processed_history_rows.append(Row(area_id=int(area_id_str), observed_at=value['observed_at']))
...
if not s3_paths_to_read:
    ...
    raw_json_df = spark.read.json(s3_paths_to_read) \
                        .withColumn("input_file_path", F.input_file_name()) 
    ...
    base_df = raw_json_df.select(
        ...
        "observed_at",
        F.regexp_replace(F.col("observed_at_raw"), "(\\d{4})(\\d{2})(\\d{2}) (\\d{2})(\\d{2})", "$1-$2-$3 $4:$5:00")
    ).withColumn(
        "source_id",
    ...
    commercial_df = base_df.select(
        ...
        F.col("AREA_SH_PAYMENT_AMT_MIN").cast(IntegerType()).alias("payment_amount_min"),
        F.col("AREA_SH_PAYMENT_AMT_MAX").cast(IntegerType()).alias("payment_amount_max"),
        ...
        F.col("CMRCL_PERSONAL_RATE").cast(DecimalType(5, 2)).alias("individual_consumer_ratio"),
        F.col("CMRCL_CORPORATION_RATE").cast(DecimalType(5, 2)).alias("corporate_consumer_ratio"),
        F.col("observed_at"),
        F.col("created_at"),
    ...
    commercial_rsb_df = base_df.withColumn(
        "rsb_data", F.explode_outer("CMRCL_RSB") # CMRCL_RSB 배열을 explode
    ).select(
        F.col("source_id"),
        ...
        F.col("rsb_data.RSB_SH_PAYMENT_CNT").cast(IntegerType()).alias("category_payment_count"),
        F.col("rsb_data.RSB_SH_PAYMENT_AMT_MIN").cast(IntegerType()).alias("category_payment_min"),
        F.col("rsb_data.RSB_SH_PAYMENT_AMT_MAX").cast(IntegerType()).alias("category_payment_max"),
    ...
    if processed_history_broadcast is not None:
        commercial_df = commercial_df.join(processed_history_broadcast,
                                            (commercial_df["area_id_for_join"] == processed_history_broadcast["area_id"]) &
                                            (commercial_df["observed_at"] == processed_history_broadcast["observed_at"]),
                                            "left_anti") 

        commercial_rsb_df = commercial_rsb_df.join(processed_history_broadcast,
                                            (commercial_rsb_df["area_id_for_join"] == processed_history_broadcast["area_id"]) &
                                            (commercial_rsb_df["observed_at"] == processed_history_broadcast["observed_at"]),
                                            "left_anti") 

    newly_processed_comm_data = commercial_df.select(F.col("area_id_for_join").alias("area_id"), "observed_at").collect()
    newly_processed_rsb_data = commercial_rsb_df.select(F.col("area_id_for_join").alias("area_id"), "observed_at").collect()
...
for row in newly_processed_comm_data + newly_processed_rsb_data:
    ...
    observed_at_str = row.observed_at.strftime("%Y-%m-%d %H:%M:%S") if isinstance(row.observed_at, datetime) else row.observed_at
    if not any(d['observed_at'] == observed_at_str for d in processed_observed_at_dict[area_id_str]):
