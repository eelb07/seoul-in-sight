...
def commercial_data_pipeline():
    ...
    def extract_and_transform(**context):
        ...
        process_start_time_kst = context['data_interval_end'].in_timezone("Asia/Seoul")
        log.info(f"🔔{process_start_time_kst} 기준 지난 12시간의 raw_json 처리를 시작합니다.")


        # 처리해야 할 전체 파일 경로 정의
        files_to_process = []

        # DAG가 오전 9시 30분에 실행되므로,
        # 조회 시작 시간은 항상 '실행일의 전날' 오후 9시 0분이 된다.
        # 예를 들어, 7월 15일 9:30 AM에 DAG이 실행되면,
        # 7월 14일 9:00 PM 부터 데이터를 조회한다. 
        start_lookup_date = process_start_time_kst.subtract(days=1).date() 
        start_lookup_time = pendulum.datetime(
            start_lookup_date.year, start_lookup_date.month, start_lookup_date.day,
            21, 0, 0, tz="Asia/Seoul" # 전날 21시 (오후 9시)
            )
        
        # 조회 종료 시간은 항상 '실행일의 당일' 오전 8시 59분이다.
        # 예를 들어, 7월 15일 9:30 AM에 DAG이 실행되면,
        # 7월 15일 8:59 AM까지 데이터를 조회한다.
        end_lookup_date = process_start_time_kst.date()
        end_lookup_time = pendulum.datetime(
            end_lookup_date.year, end_lookup_date.month, end_lookup_date.day,
            8, 59, 0, tz="Asia/Seoul" # 당일 8시 59분
        )

        log.info(f"🔔 {start_lookup_time} 부터 {end_lookup_time} 까지의 raw_json 파일을 조회합니다.")

        # 시간 단위로 순회 시작 (시작 시각의 '분'과 '초'는 무시하고 '시'부터 시작)
        current_hour_to_check = start_lookup_time.start_of("hour")
        ...
        # 시간 별 파일 순회 및 처리
        # -------------------------------------------------
        while current_hour_to_check <= end_lookup_time.start_of("hour"):
            s3_prefix_date_path = current_hour_to_check.strftime("%Y%m%d")
            # S3 Prefix를 시간(HH) 단위까지만 지정
            s3_prefix_hour_name = current_hour_to_check.strftime("%H")
            # 최종 S3 접두사: raw_json_data/20250708/00/
            full_s3_prefix = f"{S3_PREFIX}/{s3_prefix_date_path}/{s3_prefix_hour_name}"

            log.info(f"🔎 S3 prefix 조회 (시간 단위): {full_s3_prefix}")
            ...
            if 'Contents' in response:
                for obj in response['Contents']:
                    file_key = obj['Key'] # 파일 경로
                    file_name_with_ext = file_key.split('/')[-1]
                    parts_with_ext = file_name_with_ext.split('_')
                    area_id_str = parts_with_ext[1].split('.')[0] 
                    ...
                    if is_processed(area_id=area_id, observed_at=observed_at):
                        # log.info(f"⏭️ 해당 시각의 상권 데이터는 이미 처리 되었습니다. 스킵합니다. : {file_name_with_ext} (area_id: {area_id}, observed_at: {observed_at})")
                        continue
                    ...
                    log.info(f"🔔 {file_name_with_ext}의 전처리를 완료했습니다.")
        ...
        if source_commercial_data:
            ...
            # 데이터 타입 매핑 및 처리
            for row in source_commercial_data:
            ...
            # S3 업로드
            s3_key_commercial = f"{S3_PQ_PREFIX_COMM}/{date_yyyymmdd}/{time_hhmm}_night.parquet"
        ...
        if source_commercial_rsb_data:
            ...
            for row in source_commercial_rsb_data:
            ...
            # S3에 업로드
            s3_key_rsb = f"{S3_PQ_PREFIX_RSB}/{date_yyyymmdd}/{time_hhmm}_night.parquet"
